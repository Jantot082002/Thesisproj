    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4 main-content">
                <button class="return-btn" onclick="window.location.href='return_item.php'">Return Item</button>
                <div class="container mt-4">
                    <h3><b>Borrow Item</b></h3>
                    <form id="borrowForm" action="process_borrower.php" method="POST">
                        <div class="form-group" id="borrowerIdGroup">
                            <label for="borrowerId">StudentID or EmployeeID</label>
                            <input type="text" class="form-control" id="borrowerId" name="borrowerId" placeholder="Enter StudentID or EmployeeID" required>
                        </div>
                        <div class="form-group">
                            <label for="borrowerType">Borrower Type</label>
                            <input type="text" class="form-control" id="borrowerType" name="borrowerType" readonly>
                        </div>
                        <div class="form-group">
                        ...
                        <button type="submit" class="btn" style="background-color: #50B498; color: white;">Borrow</button>
                    </form>
                </div>
               <div class="container mt-4">
    <h3><b>Borrowed Items</b></h3>
...
$sql = "SELECT borrowers.id AS borrower_id, 
               products.product_name, 
               borrow_records.quantity, 
               borrow_records.borrow_date 
        FROM borrow_records
        JOIN borrowers ON borrow_records.borrower_id = borrowers.id 
        JOIN products ON borrow_records.product_id = products.id
        WHERE borrow_records.return_date IS NULL";  // Exclude returned items
...
$result = $conn->query($sql);
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                echo "<tr><td colspan='5'>No borrowed items found</td></tr>";
                            }
    ...
    <script>
        $(document).ready(function(){
            $('#borrowerId').on('blur', function(){
                var borrowerId = $(this).val();
                if(borrowerId) {
                    $.ajax({
                        url: 'fetch_borrower_type.php',
                        type: 'POST',
                        data: {borrowerId: borrowerId},
                        success: function(response){
                            $('#borrowerType').val(response);
                        },
